...
/**
 * Get deposit entry used to claim the deposit on the mainchain.
 */
export function useGetDepositEntry(
	targetNetworkId: number,
	depositId: number
) {
	const contract = useContractRead({
		...getCrossbellGatewayContractConfig(),
		functionName: "getDepositEntry",
		args: [BigNumber.from(targetNetworkId), BigNumber.from(depositId)],
		chainId: crossbellChain.id,
		onError: handleContractError,
	});
...
/**
 * Get deposit signature used to claim the deposit on the mainchain.
 */
export function useGetDepositSignature(
	targetNetworkId: number,
	depositId: number,
	...
	const contract = useContractRead({
		...getCrossbellGatewayContractConfig(),
		functionName: "getDepositSignatures",
		args: [BigNumber.from(targetNetworkId), BigNumber.from(depositId)],
		chainId: crossbellChain.id,
		onError: handleContractError,
		// cacheOnBlock: true,
